...
final class FilePond extends InputWidget
{
    ...
    /**
     * @var string|null How the File Poster image fills its panel: `'cover'` crops the image to fill the
     * whole poster box, `'contain'` fits the entire image inside it (letterboxed). `null` keeps the
     * file-poster plugin default. The upstream plugin hard-codes the poster `<img>` to cover via inline
     * styles and exposes no fit option, so this is applied through a `data-filepond-poster-fit`
     * attribute on the pond root (set in the init script) matched by the bundled widget CSS.
     */
    public string|null $filePosterObjectFit = null;
    ...
    public function init(): void
    {
        ...
        if ($this->filePosterObjectFit !== null && in_array($this->filePosterObjectFit, ['cover', 'contain'], true) === false) {
            throw new InvalidConfigException("FilePond::\$filePosterObjectFit must be 'cover', 'contain' or null.");
        }
    ...
    private function getScript(): string
    {
        ...
        // The pond root is generated by FilePond at runtime, so the poster-fit hint is stamped on it
        // here (after create) for the bundled widget CSS to match. The upstream file-poster plugin
        // exposes no fit option; this attribute is the supported way to switch cover/contain.
        $posterFit = $this->filePosterObjectFit === null
            ? ''
            : 'pond.element.setAttribute(' . Json::htmlEncode('data-filepond-poster-fit') . ', ' . Json::htmlEncode($this->filePosterObjectFit) . ")\n";
